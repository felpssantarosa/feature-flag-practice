...
export class FlagController {
	...
	initializeRoutes(fastify: FastifyInstance) {
		this.featureFlagRoutes(fastify);
	}

	private featureFlagRoutes = (fastify: FastifyInstance) => {
		const flagService = this.flagService;
		...
		fastify.get("/flags/:name", async (request, reply) => {
			...
			const flag = await this.flagService.findFlagByName(
				name,
				env ?? "production",
			);
		...
		fastify.put(
			...
			async (request, reply) => {
				...
				const created = await this.flagService.createFlag({
					name,
		...
		fastify.post(
			...
			async (request, reply) => {
				...
				const flagNames = body.flagNames ?? body.flags ?? [];
				const context =
					(
						body as {
							flags?: string[];
...
export default new FlagController(flagService, ruleService);
